...
module.exports = class EventContactController {
  ...
  static async addContactSave(req, res) {
    ...
    if (Array.isArray(contacts)) {
      await contacts.forEach(async function (Contact_contactId) {
        if (
          (await ValidateService.validateObject(
            { Event_eventId, Contact_contactId },
            "eventContact"
          )) === false
        )
          await Event_Contact.create({
            Event_eventId,
            Contact_contactId,
            UserId,
          });
      ...
      if (
        await ValidateService.validateObject(
          { Event_eventId, Contact_contactId: contacts },
          "eventContact"
        )
      ) {
  ...
  static async removeContact(req, res) {
    ...
    if (Array.isArray(contacts)) {
      ...
      await Event_Contact.destroy({
        where: { Event_eventId: Event_eventId, Contact_contactId: contacts },
      });
