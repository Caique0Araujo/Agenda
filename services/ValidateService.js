  ...
  static async validateObject(object, type) {
    ...
    switch(type) {
      case "eventContact": {
        ...
        const eventContact = await EventContact.findOne({raw: true, where: {Event_eventId: object.Event_eventId, Contact_contactId: object.Contact_contactId}})
        if(eventContact){
          return true;
        }else{
          return false;
      ...
      case "groupContact": {
        ...
        const groupContact = await GroupContact.findOne({raw: true, where: {Group_groupId: object.Group_groupId, Contact_contactId: object.Contact_contactId}})
        if(groupContact){
          return true;
        }else{
          return false;
